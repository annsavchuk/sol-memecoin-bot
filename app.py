...
def token_info(mint):
    ...
    return symbol, mc, axiom, pair_id

# ===== FORMAT =====
def format_buy(symbol, wallet, sol, mc):
...
def format_multi(symbol, data, mc):
...
def webhook():
    ...
    for tx in txs:
        ...
        for t in tx.get("tokenTransfers", []):
            ...
            symbol, mc, axiom, pair = token_info(mint)
            ...
            if not cluster or now - cluster["first_seen"] > MULTI_LIFETIME:
                ...
                if wallet not in cluster["wallets"]:
                    ...
                    if len(cluster["wallets"]) >= 2:
                        send_telegram(format_multi(symbol, cluster, mc), axiom)
