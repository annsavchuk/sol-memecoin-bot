...
# слова для фейкових USD/стейблів
FAKE_WORDS = ["usd", "usdt", "usdc", "busd", "tether", "stable", "dollar"]
...
def fetch_token_info(mint):
    try:
        ...
        # правильний Axiom link
        link = f"https://axiom.trade/meme/{mint}?chain=sol"
        return symbol, mc, link

    except:
        return mint[:6], "N/A", f"https://axiom.trade/meme/{mint}?chain=sol"

# ================== FILTER FAKE USD ==================
def is_fake_usd(symbol):
    s = symbol.lower()
    return any(w in s for w in FAKE_WORDS)

# ================== FORMAT MULTI ==================
def format_multi(mint, data):
    symbol, mc, link = fetch_token_info(mint)

    if is_fake_usd(symbol):
        return None
