...
def webhook():
    ...
    # --- DEBUG: бачимо, що реально приходить ---
    ...
    if isinstance(data, list):
        ...
        events = data.get("events") or data.get("accountData") or []
    else:
        return jsonify({"ok": True})

    if not events:
        send_telegram("⚠️ No events in payload")
        return jsonify({"ok": True})
    ...
    for tx in events:
        try:
            ...
            send_telegram(f"❌ TX parse error: {e}")
