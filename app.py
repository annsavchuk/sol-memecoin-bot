...
STABLE_KEYWORDS = ["USD", "USDT", "USDC", "DAI", "STABLE"]
...
def token_info(mint):
    ...
    if any(k in symbol.upper() for k in STABLE_KEYWORDS):
        return None, None, None

    fdv = pair.get("fdv")
    mc = f"${fdv/1000:.2f}K" if fdv else "N/A"
...
def webhook():
    ...
    for tx in txs:
        ...
        for t in tx.get("tokenTransfers", []):
            ...
            if buf and now - buf["start"] <= WALLET_BUFFER:
                ...
                if buf:
                    send_telegram(
                        format_buy(symbol, wallet, buf["total"], buf["count"], int(buf["last"]-buf["start"]), mc, mint),
                        axiom
                    )
                wallet_buffers[key] = {
                    "total": sol_amount,
